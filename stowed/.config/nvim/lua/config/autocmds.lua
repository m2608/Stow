...
autocmd({"FileType"}, {pattern = "bqn", callback = _3_})
augroup("markdown-conceal", {clear = true})
local function _5_()
  vim.opt_local.conceallevel = 2
  return nil
end
autocmd("Filetype", {group = "markdown-conceal", pattern = "markdown", callback = _5_})
...
autocmd("ColorScheme", {group = "colorscheme-patch", pattern = "default", callback = _6_})
...
autocmd({"BufReadPre", "FileReadPre", "BufNewFile"}, {group = "age-encryption", pattern = "*.age", callback = _8_})
local function _9_()
  return vim.cmd("silent % !age -d -i ~/.ssh/id_ed25519")
end
autocmd({"BufReadPost", "FileReadPost"}, {group = "age-encryption", pattern = "*.age", callback = _9_})
local function _10_()
  return vim.cmd("silent % !age -e -R ~/.ssh/id_ed25519.pub -a")
end
autocmd({"BufWritePre", "FileWritePre"}, {group = "age-encryption", pattern = "*.age", callback = _10_})
