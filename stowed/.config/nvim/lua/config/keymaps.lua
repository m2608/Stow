...
do
  ...
  _4_ = fzf.fzf_live("ast-grep run --color never --heading never --pattern <query> . 2> /dev/null", {actions = fzf.defaults.actions.files, fzf_opts = {["--multi"] = true}, keymap = {fzf = {["ctrl-q"] = "select-all+accept"}}, previewer = "builtin", exec_empty_query = false})
end
