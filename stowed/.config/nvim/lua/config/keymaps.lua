...
local function _4_()
  ...
  return fzf.fzf_live("ast-grep run --color never --heading never --pattern <query> . 2> /dev/null", {actions = fzf.defaults.actions.files, fzf_opts = {["--multi"] = true}, keymap = {fzf = {["ctrl-q"] = "select-all+accept"}}, previewer = "builtin", exec_empty_query = false})
end
mappings = {{"n", "<F2>", ":<C-u>update<CR>"}, {"i", "<F2>", "<C-o>:update<CR>"}, {"n", "<F8>", ":<C-u>AerialToggle<CR>"}, {"n", "<F9>", ":Oil --float<CR>"}, {"n", "<F10>", ":ToggleTerm<CR>"}, {"n", "<F11>", ":ZenMode<CR>"}, {"n", "<F11>", "<C-o>:ZenMode<CR>"}, {"n", "<BS>", ":nohlsearch<CR>"}, {"n", "<Leader><BS>", ":lua vim.diagnostic.hide()<CR>"}, {"n", "<Leader>e", ":e <C-R>=expand(\"%:p:h\") . \"/\"<CR>", {noremap = true}}, {"n", "<Leader>cd", ":lcd %:p:h<CR>:pwd<CR>", {noremap = true}}, {"v", "<", "<gv"}, {"v", ">", ">gv"}, {"i", "<Tab>", "pumvisible() ? \"<Down>\" : \"<Tab>\"", {noremap = true, silent = true, expr = true}}, {"i", "<S-Tab>", "pumvisible() ? \"<Up>\"   : \"\"", {noremap = true, silent = true, expr = true}}, {"i", "<C-n>", "pumvisible() ? \"<Down>\" : \"<C-x><C-o>\"", {noremap = true, silent = true, expr = true}}, {"i", "<C-p>", "pumvisible() ? \"<Up\"    : \"<C-x><C-o>\"", {noremap = true, silent = true, expr = true}}, {"i", "<CR>", "pumvisible() ? \"<c-y>\" : \"<cr>\"", {noremap = true, silent = true, expr = true}}, {"n", "Y", "y$"}, {"n", "<C-Tab>", ":wincmd w<CR>"}, {"n", "<C-S-Tab>", ":wincmd W<CR>"}, {"n", "gh", "^"}, {"n", "gl", "$"}, {"n", "<m-x>", ":", {noremap = true, silent = false}}, {"i", "<c-e>", "<c-o>$"}, {"i", "<c-a>", "<c-o>^"}, {"c", "<c-a>", "<home>", {noremap = true}}, {"c", "<c-e>", "<end>", {noremap = true}}, {"c", "<c-p>", "<up>", {noremap = true}}, {"c", "<c-n>", "<down>", {noremap = true}}, {"c", "<c-b>", "<left>", {noremap = true}}, {"c", "<c-f>", "<right>", {noremap = true}}, {"c", "<m-b>", "<s-left>", {noremap = true}}, {"c", "<m-f>", "<s-right>", {noremap = true}}, {"n", "<c-j>", ":cn<CR>"}, {"n", "<c-k>", ":cp<CR>"}, {"n", "<F3>", "yiw:vimgrep /<c-r>0/ **/*", {noremap = true}}, {"n", "gx", ":silent execute '!xdg-open ' . shellescape(expand('<cfile>'), 1)<CR>"}, {"v", "gx", "y | :silent execute '!xdg-open ' . shellescape(expand(@\"), 1)<CR>"}, {"n", "<space>f", ":FzfLua files<CR>"}, {"n", "<space>b", ":FzfLua buffers<CR>"}, {"n", "<space>/", ":FzfLua live_grep<CR>"}, {"n", "<space>g", ":FzfLua lgrep_curbuf<CR>"}, {"n", "<space>d", ":FzfLua diagnostics_document<CR>"}, {"n", "<space>s", ":FzfLua lsp_document_symbols<CR>"}, {"n", "<space>r", ":FzfLua lsp_references<CR>"}, {"n", "<space>j", ":FzfLua jumps<CR>"}, {"n", "<space>u", ":FzfLua undotree<CR>"}, {"n", "<Leader>fh", ":FzfLua helptags<CR>"}, {"n", "<Leader>fd", ":FzfLua lsp_definitions<CR>"}, {"n", "<space>a", _4_, {noremap = true, silent = true, desc = "FzfLua ast-grep"}}, {"x", "<Leader>s", "<Plug>SlimeRegionSend", {silent = true, noremap = false}}, {"n", "<Leader>s", "<Plug>SlimeParagraphSend", {silent = true, noremap = false}}, {"t", "<Esc>", "<C-\\><C-n>"}, {"n", "<space>y", ":OSCYankOperator<CR>", {silent = true, noremap = false}}, {"v", "<space>y", ":OSCYankVisual<CR>", {silent = true, noremap = false}}, {"n", "<space>h", ":w !xclip -selection clipboard -t text/html<CR><CR>", {silent = true, noremap = false}}, {"v", "<space>h", ":w !xclip -selection clipboard -t text/html<CR><CR>", {silent = true, noremap = false}}, {"n", "<space>p", "\"+p"}, {"n", "<space>P", "\"+P"}, {"n", "<A-h>", "<CMD>NavigatorLeft<CR>"}, {"n", "<A-l>", "<CMD>NavigatorRight<CR>"}, {"n", "<A-k>", "<CMD>NavigatorUp<CR>"}, {"n", "<A-j>", "<CMD>NavigatorDown<CR>"}}
