...
config.merge({client = {joker = {stdio = {command = "joker --no-readline", prompt_pattern = "user=> "}}}})
if config["get-in"]({"mapping", "enable_defaults"}) then
  config.merge({client = {joker = {stdio = {mapping = {start = "cs", stop = "cS", interrupt = "ei"}}}}})
else
end
local cfg = config["get-in-fn"]({"client", "joker", "stdio"})
local state
local function _3_()
  return {repl = nil}
end
...
local function on_filetype()
  mapping.buf("JkStart", cfg({"mapping", "start"}), start, {desc = "Start the REPL"})
  mapping.buf("JkStop", cfg({"mapping", "stop"}), stop, {desc = "Stop the REPL"})
  return mapping.buf("JkInterrupt", cfg({"mapping", "interrupt"}), interrupt, {desc = "Interrupt the REPL"})
end
