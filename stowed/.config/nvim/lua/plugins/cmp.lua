...
local function _1_()
  ...
  return cmp.setup({sources = cmp.config.sources(core.map(_2_, {"conjure", "nvim_lsp", "buffer", "latex_symbols"})), completion = {autocomplete = false}, mapping = {["<C-n>"] = cmp.mapping.select_next_item({behavior = cmp.SelectBehavior.Select}), ["<C-p>"] = cmp.mapping.select_prev_item({behavior = cmp.SelectBehavior.Select}), ["<CR>"] = cmp.mapping.confirm({select = true}), ["<Space>"] = cmp.mapping(_3_, {"i", "s"})}})
end
